...
export const PosterCard: FC<IPoster> = (poster) => {
    ...
    return (
        <Card className='w-[35rem] px-2' radius='lg' shadow='md'>
            <CardHeader className='flex justify-between'>
                <h4 className='font-semibold text-default-700'>
                    From: &nbsp;
                    <Link isExternal color='foreground' href={`${sepoliaEtherscanURL}/address/${user}`}>
                        {sliceAddress(user as Address)}
                    </Link>
                </h4>
                <Link isExternal showAnchorIcon href={`${sepoliaEtherscanURL}/tx/${transactionHash}`}>
