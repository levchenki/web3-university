            ...
            <div className='flex flex-row gap-3 items-center'>
                ...
                {isConnected ? (
                    <div className='flex flex-col'>
                        ...
                        {isTokenLoading ? (
                            <h3 className='text-default-400 font-italic text-sm'>Loading...</h3>
                        ) : (
                            <h3 className='text-default-400 font-italic text-sm'>{balanceString} PDG</h3>
                        )}
                    </div>
                ) : (
                    <h3 className='text-default-400 font-italic text-sm'>Not connected</h3>
                )}
                <Button isIconOnly size='sm' variant='flat' color='secondary' disabled={!isConnected} onClick={onOpen}>
                    <MdCurrencyExchange />
                </Button>
                <Button
                    isIconOnly
